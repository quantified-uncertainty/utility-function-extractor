...
function findDistancesForAllElements({nodes, links}){
    ...
    let distances = nodes.map(element =>
        element.isReferenceValue ? [1] : findDistance({sourceElementId: referenceElement.id, targetElementId: element.id, nodes, links})
    )
    return distances
}

export function CreateTableWithDistances({isListOrdered, nodes, links}){
    if(!isListOrdered){ 
        return (<div>{""}</div>)
    } else {
