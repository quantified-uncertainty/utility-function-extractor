...
let displayFunctionSlider = (value) => {
  let result
  if (value >= 0) {
    result = transformSliderValueToPracticalValue(value)
  } else {
    let inverseresult = transformSliderValueToPracticalValue(-value)
...
export async function getStaticProps() {
  ...
  let listOfMoralGoods = JSON.parse(fs.readFileSync(path.join(directory, 'listOfMoralGoods.json'), 'utf8'));
  //console.log(directory)
  //console.log("metaforecasts", metaforecasts)
  return {
    props: {
      listOfElementsDefault: listOfMoralGoods,
    },
  };
}
